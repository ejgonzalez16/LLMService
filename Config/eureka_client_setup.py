from py_eureka_client import eureka_client
import psutil

# Dirección del servidor Eureka
EUREKA_SERVER = "http://10.43.102.146:8761/eureka/"


# Nombre exacto de tu adaptador
target_substr = "javeriana"  # parte del nombre que sabemos que contiene
addrs = psutil.net_if_addrs()

found = False
ip = "localhost"
for adapter_name, adapter_addrs in addrs.items():
    if target_substr in adapter_name:  # busca adaptador que contenga "javeriana"
        for addr in adapter_addrs:
            if addr.family.name == "AF_INET":  # IPv4
                print(f"IP de {adapter_name}: {addr.address}")
                ip = addr.address
                found = True

# Registrar el microservicio
eureka_client.init(
    eureka_server=EUREKA_SERVER,
    app_name="llm-service",      # Nombre de tu servicio
    instance_port=8001,        # Puerto de Django
    instance_host=ip, # Host de la instancia
    renewal_interval_in_secs=10,
    duration_in_secs=30,
)